...
// collEditor is the focused sub-panel that edits a FieldList or FieldMap leaf: a small
// list of rows the user walks, edits in place, and grows/shrinks with add/remove. It owns
// no disk state — it parses the leaf's TOML literal on open and re-serializes it on done,
// so the Editor's Change/write-back path is identical to the old raw-text edit; the widget
// only stops you fat-fingering the brackets and quotes.
//
// Modeless typing collides with row commands (a node named "node-d" versus `d` = delete),
// so this widget IS modal: nav mode walks rows (a add, d remove, ← → pick a map column,
// ↵ edit the cell), and an explicit edit mode takes the keystrokes. It's the one place the
// house's modeless rule doesn't fit — the collision is real, not stylistic.
type collEditor struct {
	isMap   bool
	label   string
	rows    []collRow
	cursor  int // 0..len(rows); == len(rows) is the "+ add" row
...
type collRow struct{ key, val string }

// newCollEditor seeds the rows from a leaf's literal (an empty/unset leaf opens empty).
func newCollEditor(kind FieldKind, label, lit string) *collEditor {
	e := &collEditor{isMap: kind == FieldMap, label: label}
	if e.isMap {
		for _, p := range parseMap(lit) {
			e.rows = append(e.rows, collRow{p[0], p[1]})
		}
	} else {
...
// update applies one keystroke. In edit mode every rune lands in the cell (so a value may
// contain any letter); nav mode reads a/d/arrows as commands.
